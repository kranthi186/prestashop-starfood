...
$(document).ready(function() {
	...
	$('img.js-disabled-action').css({"opacity":0.5});
        ...
        // submit of add template form
        $('#addInoiceForm').submit(function (event) {
...
function addViewOrderDetailRow(view)
{
	...
	$('tr#new_invoice').hide();
	$('tr#new_product').hide();
	$('tr#new_product_search_block').hide();

	// Initialize fields
	closeAddProduct();

	$('tr#new_product_search_block').before(html);
...
function init()
{
	$('#txt_msg').on('keyup', function(){
        ...
        $('.inStockChangeLink').on('click', function(e){
            e.preventDefault();
        ...
        $('.shippedChangeLink').on('click', function(e){
            e.preventDefault();
            var orderDetailId = $(this).parent().parent().find('input.productIdCheckBox').val();
            if (!orderDetailId)
            {
                orderDetailId = $(this).parent().parent().parent().find('input.productIdCheckBox').val();
            }
            var clickedA = this;
            $.post(admin_order_tab_link, {id: orderDetailId, ajax: 1, action: 'toggleShippedState'}, function(result){
                    if (result.error)
                    {
                        jAlert(result.error);
                    }
	...
	$('#add_product').unbind('click').click(function(e) {
		...
		$('tr#new_product_search_block').slideDown('fast', function () {});
	...
	.result(function(event, data, formatted) {
		...
		else
		{
			...
			$('#add_product_product_quantity').focus();
		}
	});
	...
	$('#add_product_product_quantity').keydown(function(event){
		if(event.which == 13){
			event.preventDefault();
	...
	$('#submitAddProduct').unbind('click').click(function(e) {
		...
		if (go)
		{
			...
			if (go)
			{
				var query = 'ajax=1&action=addProductOnOrder&id_order='+id_order+'&';
...
var currentSearchProductsList = [];
function newProductSearchAdd(event){
	console.log(event);
	var selectedProductId = $(this).data('product_id');
	var selectedAttributeId = $(this).data('attribute_id');
	console.log(selectedProductId, selectedAttributeId);
	var data = {};
	for(var i in currentSearchProductsList){
		if( currentSearchProductsList[i].id_product == selectedProductId ){
			if(selectedAttributeId == 0){
				data = currentSearchProductsList[i];
				break;
			}
			else{
				// if there are no combinations, then exit
				if( currentSearchProductsList[i].combinations.length == 0 ){
					break;
				}
				data = currentSearchProductsList[i];
				for(var y in currentSearchProductsList[i].combinations){
					if( currentSearchProductsList[i].combinations[y].id_product_attribute == selectedAttributeId ){
						data.combination = currentSearchProductsList[i].combinations[y];
					}
				}
			}
		}
	...
	if( typeof data.combination == 'undefined' ){
		...
		$('#add_product_product_price_tax_incl').val(data.combination.price_tax_incl);
		$('#add_product_product_price_tax_excl').val(data.combination.price_tax_excl);
		if (stock_management)
			$('#add_product_product_stock').html(data.combination.qty_in_stock);
	...
	if (typeof data.combination !== 'undefined')
	{
		...
		$('select#add_product_product_attribute_id').append(
			'<option value="'+data.combination.id_product_attribute+'">'+data.combination.attributes+'</option>');
		// Show select list
		$('#add_product_product_attribute_area').show();

		//populateWarehouseList(data.warehouse_list[defaultAttribute]);
	}
	else
	{
		// Reset combinations list
	...
	$.ajax({
		...
		success: function(res){
			...
			if(!res.found){
				...
				currentSearchProductsList = [];
				return;
			}
			
			$('#products_table').on('click', 'button.new_product_search_add_product', newProductSearchAdd);
			
			$('#products_found').show();
			var productsTable = $('#products_table');
			productsTable.find('tr.product-row').remove();
			currentSearchProductsList = res.products;
				...
				if(this.combinations.length){
					for(var ci in this.combinations){
						...
						productRow.append($('<td/>').append(
							$('<button/>').attr({ type:'button',class:'btn btn-xs new_product_search_add_product' })
							.text('Choose').data({ product_id:this.id_product,attribute_id:this.combinations[ci].id_product_attribute })
						));
					...
					productRow.append($('<td/>').append(
						$('<button/>').attr({ type:'button',class:'btn btn-xs new_product_search_add_product' })
						.text('Choose').data({ product_id:this.id_product,attribute_id:0 })
					));
