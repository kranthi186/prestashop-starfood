...
function orderOverwriteMessage(sl, text)
{
	...
	if (sl_value != '0')
	{
		if ($zone.val().length > 0 && !confirm(text))
			return ;
		$zone.val(sl_value);
                $('#custMsgSubject').val(sl.options.item(sl.selectedIndex).text);
	}
...
function sendBulkAction(form, action, targetBlank)
{
        var form_action = $(form).attr('action');
	if (targetBlank)
	{
            // if targetBlank contains action
            if (targetBlank!='1')
            {
                // send form with target blank action in new window
                sendBulkAction(form, targetBlank, '1');
                // continue and send current action as usual
	...
	$(form).submit();
        ...
        // if target blank, restore form action
