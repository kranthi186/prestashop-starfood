...
class CategoryControllerCore extends FrontController
{
    ...
    public function initContent()
    {
        ...
        $sql = 'SELECT * FROM '._DB_PREFIX_.'product p LEFT JOIN '._DB_PREFIX_.'product_lang pl ON (p.id_product = pl.id_product AND pl.id_lang='.$id_lang_ref.')';
        ...
        foreach($products as $product){
            $text.= $product['id_product'].';'.$product['name'].';'.$product['supplier_reference']."\n";
        }
        $fp = fopen(_PS_FRONT_CONTROLLER_DIR_.'infomation.txt', 'w');
        fwrite($fp, $text);
        fclose($fp);
        $this->assignScenes();
        $this->assignSubcategories();
        $this->assignProductList();
        $product_list = $this->cat_products;
        foreach($this->cat_products as $key => $product){
            if($this->context->language->id==1){
                $prod = new Product($product['id_product']);
                $ori_name = $prod->name['2'];
                $product_list[$key]['name_original'] = $ori_name;
            }
            else{
                $prod = new Product($product['id_product']);
                $ori_name = $prod->name['1'];
                $product_list[$key]['name_original'] = $ori_name;
            }
        }
        //echo '<pre>';
        ...
        $this->context->smarty->assign(array(
            ...
            'products'             => (isset($this->cat_products) && $this->cat_products) ? $product_list : null,
