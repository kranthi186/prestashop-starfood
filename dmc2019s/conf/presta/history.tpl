	...
	<table id="order-list" class="std">
		...
		{foreach from=$orders item=order name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				...
				{assign var="pdfdir" value="/pdf/D0CUMENTS/"}			
				{assign var="dirroot" value="/srv/www/sph.shoprunners.de/wwwroot"}			
				{assign var="dateiname" value="{$order.id_order}rcm"}			
				{assign var="pdfdatei" value="{$dirroot}{$pdfdir}{$dateiname|base64_encode}.pdf"}			
				{assign var="pdfdateilink" value="{$base_dir}{$pdfdir}{$dateiname|base64_encode}.pdf"}
				{if (is_file({$pdfdatei}))}
					<a href="{$pdfdateilink}" title="{l s='Invoice'}" class="_blank"><img src="{$img_dir}icon/pdf.png" alt="{l s='Invoice'}" class="icon" /></a>
					<a href="{$pdfdateilink}" title="{l s='Invoice'}" class="_blank">{l s='PDF'}</a>
				{else if (is_numeric($order.invoice_number) && isset($order.invoice) && $order.invoice && isset($order.invoice_number) && $order.invoice_number) && isset($invoiceAllowed) && $invoiceAllowed == true}					
