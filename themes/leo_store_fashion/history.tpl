...
<div class="block-center table-responsive" id="block-history">
	{if $orders && count($orders)}
		<table id="order-list" class="table table-bordered footab table-account">
			<thead>
				<tr>
					...
					<th data-sort-ignore="true" data-hide="phone,tablet" class="item">{l s='Invoice(s)'}</th>
			...
			<tbody>
				{foreach from=$orders item=order name=myLoop}
					<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
						...
						{if isset($order.invoices_list) && count($order.invoices_list)}
                            {foreach $order.invoices_list as $order_invoice}
                                <a  class="link-button" href="{$link->getPageLink('pdf-invoice', true, NULL, "id_order_invoice={$order_invoice->id}")|escape:'html':'UTF-8'}" title="{l s='Invoice'}" target="_blank"
                                  ><i class="fa fa-file-text large"></i> {$order_invoice->number_formatted}</a>
                                <br>
                            {/foreach}
						{else}
							-
						...
						<td class="history_detail">
							<a class="btn btn-outline button button-small btn-sm" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true, NULL, "id_order={$order.id_order|intval}")|escape:'html':'UTF-8'}');">
								<span>
									{l s='Details'}
								</span>
							</a>
