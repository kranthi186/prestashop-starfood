...
<h1 class="page-heading bottom-indent size24">
...
{else}
	...
	<form action="{$request_uri}" method="post" class="contact-form-box" enctype="multipart/form-data">
		<fieldset>
			<h3 class="page-subheading size20">{l s='send a message'}</h3>
			<div class="clearfix">
				<div class="col-xs-12 col-md-4 ">
					...
					{else}
						<select id="id_contact" class="form-control" name="id_contact">
							{foreach from=$contacts item=contact}
								<option value="{$contact.id_contact|intval}"{if $contact.id_contact==2} selected="selected"{/if}>{$contact.name|escape:'html':'UTF-8'}</option>
							{/foreach}
						</select>
					</div>
				...
				<div class="col-xs-12 col-md-12">
					<div class="form-group">
                        ...
                        {assign var="modulesDir" value=$smarty.const._PS_MODULE_DIR_}
                        {include file="$modulesDir./captchadd/views/templates/hook/captchadd.tpl"}
			<div class="submit">
				<button type="submit" style="margin-left:15px;" name="submitMessage" id="submitMessage" class="button btn btn-outline button-medium"><span>{l s='Send'}</span></button>
			</div>
		</fieldset>
	</form>
{/if}
