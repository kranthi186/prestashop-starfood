...
{capture name=path}
	...
	{/if}
{/capture}
<h1 class="page-heading hidden">{if !isset($email_create)}{l s='Authentication'}{else}{l s='Create an account'}{/if}</h1>
...
{if !isset($email_create)}
	...
	<div class="row loginform">
		...
		<div class="col-xs-12 col-sm-6 loginform2">
		...
		<form action="{$link->getPageLink('authentication', true, NULL, "back=$back")|escape:'html':'UTF-8'}" method="post" id="new_account_form" class="std clearfix form-horizontal">
			<div class="box">
				<div id="opc_account_form" style="display: block; ">
					<h1 class="page-subheading bottom-indent" style="font-size:26px;">{l s='Instant checkout'}</h1>
...
{else}
	...
	{/if}-->
    <div class="col-xs-12 ">
       <h1 class="page-subheading" style="font-size:26px;">{l s='Neukundenregistrierung'}</h1>

        {hook h='MyTextBlockHook'}
    </div>
	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="account-creation_form" class="std box form-horizontal">
		...
		<div class="account_creation">
            ...
            <div class="form-group" style="float:none;" >
    					<label class="control-label col-sm-12" for="">{l s='Company'}</label>
    					<div class="col-sm-12" >
    						<input type="text" class="form-control" id="company" name="company" value="{if isset($smarty.post.company)}{$smarty.post.company}{/if}" />
    					</div>
			</div>
			{if $field_name eq 'year_open'}
            <div class="form-group">
                       <label class="control-label col-sm-12" for="year_open">{l s='Gründungsjahr'}</label>
    					<div class="col-sm-12">
    						<input type="text" class="form-control" id="year_open" name="year_open" value="{if isset($smarty.post.ape)}{$smarty.post.ape}{/if}" />
    					</div>
         	</div>
         	{/if}
			<div class="required form-group">
				<label class="control-label col-sm-12" for="customer_firstname">{l s='First name'} <sup>*</sup></label>
				<div class="col-sm-12">
			...
			<div class="required form-group">
				<label class="control-label col-sm-12" for="customer_lastname">{l s='Last name'} <sup>*</sup></label>
				<div class="col-sm-12">
			...
			<div class="required form-group hidden">
				<label class="control-label col-sm-12" for="email">{l s='Email'} <sup>*</sup></label>
				<div class="col-sm-12">
				<input type="email" autocomplete="off" class="is_required validate form-control" data-validate="isEmail" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email}{/if}" />
				</div>
			</div>
			<div class="required password form-group">
				<label class="control-label col-sm-12" for="passwd">{l s='Password'} <sup>*</sup></label>
				<div class="col-sm-12">
					<input type="password" class="is_required validate form-control" data-validate="isPasswd" name="passwd" id="passwd" />
					{*}<span class="form_info">{l s='(Five characters minimum)'}</span>*}
				</div>
			</div>
            <div class="required password_conf form-group">
				<label class="control-label col-sm-12" for="passwd_conf">{l s='password confirm'} <sup>*</sup></label>
				<div class="col-sm-12">
					<input type="password" class="is_required validate form-control" data-validate="isPasswd" name="passwd_conf" id="passwd_conf" />
					{*}<span class="form_info">{l s='(Five characters minimum)'}</span>*}
				</div>
			</div>
                {*if $b2b_enable}
    				<div class="form-group">
    					<label class="control-label col-sm-12" for="siret">{l s='Umsatzsteuer Id Nummer'}</label>
    					<div class="col-sm-12">
    						<input type="text" class="form-control" id="siret" name="siret" value="{if isset($smarty.post.siret)}{$smarty.post.siret}{/if}" />
    					</div>
    				</div>
    				<div class="form-group">
    					<label class="control-label col-sm-12" for="ape">{l s='Handelsregisternummer / Ort'}</label>
    					<div class="col-sm-12">
    		...
    		{/if*}
            <div class="form-group">
					<label class="control-label col-sm-12" for="phone">{l s='Home phone'}{if isset($one_phone_at_least) && $one_phone_at_least} <sup>*</sup>{/if}</label>
					<div class="col-sm-12">
						<input type="text" class="form-control" name="phone" id="phone" value="{if isset($smarty.post.phone)}{$smarty.post.phone}{/if}" />
					</div>
			</div>
            <div class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group">
    					<label class="control-label col-sm-12" for="phone_mobile">{l s='Fax'}{if isset($one_phone_at_least) && $one_phone_at_least} {/if}</label>
    					<div class="col-sm-12">
    						<input type="text" class="form-control" name="phone_mobile" id="phone_mobile" value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{/if}" />
    					</div>
			</div>
                ...
                <div class="form-group">
    					<label class="control-label col-sm-12" for="website">{l s='Website'}</label>
    					<div class="col-sm-12">
		...
		{if isset($PS_REGISTRATION_PROCESS_TYPE) && $PS_REGISTRATION_PROCESS_TYPE}
			<div class="account_creation" style="margin-bottom:10px;">
				<h2 class="page-subheading" style="font-size:22px;">{l s='Ladengeschäft'}</h2>
                <div class="account_box" style="margin-bottom:10px">
				{foreach from=$dlv_all_fields item=field_name}
					{if $field_name eq "firstname"}
						<div class="required form-group hidden">
							<label class="control-label col-sm-12" for="firstname">{l s='First name'} <sup>*</sup></label>
							<div class="col-sm-12">
					...
					{elseif $field_name eq "lastname"}
						<div class="required form-group hidden">
							<label class="control-label col-sm-12" for="lastname">{l s='Last name'} <sup>*</sup></label>
							<div class="col-sm-12">
								<input type="text" class="form-control" id="lastname" name="lastname" value="{if isset($smarty.post.lastname)}{$smarty.post.lastname}{/if}" />
							</div>
						</div>
					{elseif $field_name eq "address1"}
						<div class="required form-group" style="width:100%;">
							<label class="control-label col-sm-12" for="address1">{l s='Address'} <sup>*</sup></label>
					...
					{elseif $field_name eq "address2"}
						<div class="form-group is_customer_param hidden">
							<label class="control-label col-sm-12 hidden" for="address2">{l s='Address (Line 2)'}{if in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
					...
					{elseif $field_name eq "postcode"}
						{assign var='postCodeExist' value=true}
						<div class="required postcode form-group">
							<label class="control-label col-sm-12" for="postcode">{l s='Zip / Postal Code'} <sup>*</sup></label>
							<div class="col-sm-12">
							<input type="text" class="validate form-control" name="postcode" id="postcode" data-validate="isPostCode" value="{if isset($smarty.post.postcode)}{$smarty.post.postcode}{/if}"/>
							</div>
						</div>
					{elseif $field_name eq "city"}
						<div class="required form-group">
							<label class="control-label col-sm-12" for="city">{l s='City'} <sup>*</sup></label>
							<div class="col-sm-12">
								<input type="text" class="form-control" name="city" id="city" value="{if isset($smarty.post.city)}{$smarty.post.city}{/if}" />
							</div>
						</div>
						<!-- if customer hasn't update his layout address, country has to be verified but it's deprecated -->
					{elseif $field_name eq "Country:name" || $field_name eq "country"}
						<div class="required select form-group" style="width:100%;">
							<label class="control-label col-sm-12" for="id_country">{l s='Country'} <sup>*</sup></label>
							<div class="col-sm-6" style="width:48%;">
					...
					{elseif $field_name eq "State:name" || $field_name eq 'state'}
						{assign var='stateExist' value=true}
						<div class="required id_state select form-group">
							<label class="control-label col-sm-12" for="id_state">{l s='State'} <sup>*</sup></label>
							<div class="col-sm-12">
				...
				{/if}
				<div class="textarea form-group hidden" style="display:none;">
					<label class="control-label col-sm-12" for="other">{l s='Additional information'}</label>
					<div class="col-sm-12">
				...
				<div class="form-group required ">
    					<label class="control-label col-sm-12" for="reference_brand">{l s='Referenz Marken '}<sup>*</sup></label>
    					<div class="col-sm-12">
    						<input type="text" class="form-control" id="reference_brand" name="reference_brand" value="{if isset($smarty.post.reference_brand)}{$smarty.post.reference_brand}{/if}" />
    					</div>
				</div>
                <div class="form-group">
    					<label class="control-label col-sm-12" for="sitze-shop">{l s='VerkaufsflÃ¤che Ihres LadengeschÃ¤fts '}<sup>*</sup></label>
    					<div class="col-sm-12">
    						<input type="text" class="form-control" id="sitze-shop" name="size_shop" value="{if isset($smarty.post.size_shop)}{$smarty.post.size_shop}{/if}" />
    					</div>
				</div>
                {if isset($one_phone_at_least) && $one_phone_at_least}
				...
				{/if}
				<div class="required form-group" id="address_alias hidden" style="display:none;">
					<label class="control-label col-sm-12" for="alias">{l s='Assign an address alias for future reference.'} <sup>*</sup></label>
					<div class="col-sm-12">
			...
			<div class="account_creation dni">
				...
				<div class="required form-group">
					<label class="control-label col-sm-12" for="dni">{l s='Identification number'} <sup>*</sup></label>
					<div class="col-sm-12">
		...
		{/if}
        <p>{l s='Mit Ihrer Registrierung bestÃ¤tigen Sie, dass Sie im B2B-Shop ausschliesslich fÃ¼r den Wiederverkauf und nicht fÃ¼r den Eigenbedarf kaufen werden.'}</p>
		{$HOOK_CREATE_ACCOUNT_FORM}
		<div class="submit clearfix">
			<input type="hidden" name="email_create" value="1" />
			<input type="hidden" name="is_new_customer" value="1" />
			{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'html':'UTF-8'}" />{/if}
			<button type="submit" name="submitAccount" id="submitAccount" class="btn btn-outline button button-medium" onclick="checkPassword();">
               ...
               <script type="text/javascript">
                function checkPassword(){
                    if($('#passwd_conf').val()!=$('#passwd').val())
                        {
                            $('#passwd_conf').css('border-color','red');
                            return false;
                        }
                }
            </script>{/literal}
