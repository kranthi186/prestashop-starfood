	...
	<meta itemprop="url" content="{$link->getProductLink($product)}">
		...
		<div class="pb-left-column col-xs-12 col-sm-12 col-md-7">
			...
			<div id="image-block" class="clearfix">
				<div class="p-label">
					{if $product->new}					
						<span class="labelnew label"><span class="label-new">{l s='New'}</span></span>					
					{/if}
					{if $product->on_sale}					
						<span class="labelsale label"><span class="label-sale">{l s='Sale!'}</span></span>					
				...
				{if $have_image}
					<span id="view_full_size">
						{if $jqZoomEnabled && $have_image && !$content_only}
							<a class="jqzoom" title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" rel="gal1" href="{$link->getImageLink($product->link_rewrite, $cover.id_image, '')|escape:'html':'UTF-8'}">
		...
		<div class="pb-center-column col-xs-12 col-sm-12 col-md-5">
			...
			<h1 itemprop="name">{$product->name|escape:'html':'UTF-8'}
           {*<br /><sub>{l s='PS ID: '}{$product->id|escape:'html':'UTF-8'}</sub>*}
           </h1>
			...
			<p id="supplier_reference" {if  !$product->supplier_reference}style="display: none;"{/if}><label for="manufacturer_name">{l s='Supplier Reference :'} </label><span class="editable" id="supplier_reference_number">{$product->supplier_reference|escape:'htmlall':'UTF-8'}</span>
			<script>
			$(document).ready(function(){
				$('#supplier_reference #supplier_reference_number').text($('#supplier_reference #supplier_reference_number').text().replace(/\D/g, ''));
				});
			</script>
			</p>
							...
							<div class="price">
								<p class="our_price_display" itemprop="offers" itemscope itemtype="https://schema.org/Offer">{strip}
									...
									{if $priceDisplay >= 0 && $priceDisplay <= 2}
										...
										<span style="font-size:12px;">{if $tax_enabled  && ((isset($display_tax_label) && $display_tax_label == 1) || !isset($display_tax_label))}
											{if $priceDisplay == 1} {l s='tax excl. und zzgl.'}
											<a href="javascript:void(0)" class="call-cms-page-popup"> {l s='Versandkosten'}</a>
											<div class="cms-page-popup" style="display:none">
												<div class="cms-page-popup-wrap">
												<div class="cms-page-popup-close"></div>
												<div class="to-scroll-y">
												{$product->getCMSContent(5,$cookie->id_lang)}</div>
												</div>
											</div>
					<script>
					$(document).ready(function(){
						$('.cms-page-popup').hide();
						$('.call-cms-page-popup').click(function(){
							$('.cms-page-popup').show();
						});
						$('.cms-page-popup .cms-page-popup-close').click(function(){
							$('.cms-page-popup').hide();
						});
						$(document).mouseup(function (e) {
							if ($('.cms-page-popup-wrap').has(e.target).length === 0){
								$('.cms-page-popup').hide();
							}
						});
					});
			...
			{/if}
			{hook h='productVariants' productId=$product->id productDetails=1}
		</div>
		<!-- end center infos-->
		<!-- pb-right-column1 -->
		<div class="pb-right-column col-xs-12 col-sm-12 col-md-5">
			...
			<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
                ...
                <p {if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="our_price_display show"{else}class="unvisible" {/if}>
                {l s='Preis:'} <a href="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" style="font-size:14px;">{l s='"Sichtbar als registrierter HÃ¤ndler"'}</a>
                </p>
				...
				<div class="box-info-product">
												...
												{if ($group.group_type == 'select')}
														<select class="form-control attribute_select no-print" name="{$groupName}" id="group_{$id_attribute_group|intval}" >
															{foreach from=$group.attributes key=id_attribute item=group_attribute} 
																<option value="{$id_attribute|intval}"{if (isset($smarty.get.$groupName) && $smarty.get.$groupName|intval == $id_attribute) || !isset($smarty.get.$groupName) && $group.default == $id_attribute} selected="selected"{/if} title="{$group_attribute|escape:'html':'UTF-8'}">{$group_attribute|escape:'html':'UTF-8'}</option>
															{/foreach}
														</select>
					...
					<div class="box-cart box-cart-bottom">
                        ...
                        <div {if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="show"{else}class="unvisible" {/if}>
                            <a class="exclusive btn btn-default " id="show_mes" href="#show_message">									 
									<span>{l s='Add to cart'}</span>
							</a>
                            <div style="display:none;" id="show_message" class="box-cart">{l s='Sie müssen registrierter und freigeschalteter Kunde bei uns sein um Preise zu sehen und Bestellungen aufzugeben.'} <br /><br /><a class="exclusive btn btn-default" href="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}">{l s='Jetzt anmelden'}</a></div>
                        </div>
						...
						<a class="add_to_wishlist btn btn-default buttons_bottom_block" href="#" onclick="WishlistCart('wishlist_block_list', 'add', '{$product->id|intval}', $('#idCombination').val(), document.getElementById('quantity_wanted').value); return false;" rel="nofollow"  title="{l s='Add to my wishlist'}">
							{l s='Add to wishlist'}
						</a>*}
					</div> <!-- end box-cart -->
                    {if isset($groups)}
                    <div class="clear box-attribute-infomation">
                        <table class="table table-responsive">
                            ...
                            <tr>
                                <td>{$group.name}</td>
                                {foreach from = $group.attributes key = id_attribute item=value}
                            ...
                            <tr>
                                <td>{l s='Available stock'}</td>
                                {foreach from = $group.attributes key = id_attribute item=value}
                                    <td>
                                        {if $group.attributes_quantity.$id_attribute <= 0}
                                            <span class="available_status out_stock " title="{l s='Orange'}">
                                            </span>
                                        {elseif $group.attributes_quantity.$id_attribute <=2}
                                            <span class="available_status lower_stock " title="{l s='Yellow'}">
                                            </span>
                                        {elseif $group.attributes_quantity.$id_attribute > 2}
                                            <span class="available_status in_stock " title="{l s='Green'}">
                                            </span>
                                        {/if}
                                    </td>
                               {/foreach}
                        ...
                        <table class="table table-responsive">
                            <tr>
                            	...
                            	<td>
                            		<span title="{l s='Green'}">= {l s='In stock'}</span>
							...
							<tr>
								...
								<td>
									<span class="" title="{l s='Orange'}">
										= {l s='out of stock / no stock available'}
									</span>
								</td>
							</tr>
							</table>
	...
	<div id="layer_cart">
		<div class="clearfix">
			<div class="layer_cart_product col-xs-12 col-md-6">
				...
				<span class="title">
					<i class="fa fa-ok"></i>{l s='Product successfully added to your shopping cart'}
				</span>
				...
				<div class="layer_cart_product_info">
					...
					<div>
						<strong class="dark">{l s='Quantity'}</strong>
						<span id="layer_cart_product_quantity"></span>
					</div>
					<div>
						<strong class="dark">{l s='Total'}</strong>
			...
			<div class="layer_cart_cart col-xs-12 col-md-6">
				<span class="title">
					...
					<span class="ajax_cart_product_txt_s {if $cart_qties < 2} unvisible{/if}">
						{l s='There are [1]%d[/1] items in your cart.' sprintf=[$cart_qties] tags=['<span class="ajax_cart_quantity">']}
					</span>
				...
				<div class="layer_cart_row">
					<strong class="dark">
						...
						{if $use_taxes && $display_tax_label && $show_tax}
							{if $priceDisplay == 1}
								<span class="sbh">{l s='(tax excl.)'}<span class="sbh">
							{else}
								{l s='(tax incl.)'}
							{/if}
						{/if}
				...
				{if $show_wrapping}
					<div class="layer_cart_row">
						<strong class="dark">
							...
							{if $use_taxes && $display_tax_label && $show_tax}
								{if $priceDisplay == 1}
									<span class="sbh">{l s='(tax excl.)'}</span>
								{else}
									{l s='(tax incl.)'}
								{/if}
							{/if}
				...
				<div class="layer_cart_row">
					<strong class="dark{if $shipping_cost_float == 0 && (!$cart_qties || $cart->isVirtualCart() || !isset($cart->id_address_delivery) || !$cart->id_address_delivery)} unvisible{/if}">
						{l s='Total shipping'}&nbsp;{if $use_taxes && $display_tax_label && $show_tax}{if $priceDisplay == 1}<span class="sbh">{l s='(tax excl.)'}<span>{else}{l s='(tax incl.)'}{/if}{/if}
					</strong>
					<span class="ajax_cart_shipping_cost{if $shipping_cost_float == 0 && (!$cart_qties || $cart->isVirtualCart() || !isset($cart->id_address_delivery) || !$cart->id_address_delivery)} unvisible{/if}">
						{if $shipping_cost_float == 0}
							 {if (!isset($cart->id_address_delivery) || !$cart->id_address_delivery)}{l s='To be determined'}{else}{l s='Free shipping!'}{/if}
						{else}
							{$shipping_cost}
						{/if}
					</span>
				...
				{if $show_tax && isset($tax_cost)}
					<div class="layer_cart_row">
						<strong class="dark">{l s='Tax'}</strong>
				...
				<div class="button-container">	
					<a class="continue btn btn-warning button pull-righ" title="{l s='Continue shopping'}" href="javascript:void(0)">
						<span>
							{l s='Continue shopping'}
						</span>
					</a>
					<a class="btn btn-warning button pull-right" href="{$link->getPageLink("order", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout'}" rel="nofollow">
						<span>
