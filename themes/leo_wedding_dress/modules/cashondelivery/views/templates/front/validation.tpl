...
<form action="{$link->getModuleLink('cashondelivery', 'validation', [], true)|escape:'html'}" method="post">
    ...
    <p class="cart_navigation" id="cart_navigation">
        <a href="{$link->getPageLink('order', true)}?step=3" class="button-exclusive btn btn-outline pull-right" >{l s='Other payment methods' mod='cashondelivery'}</a>
        <button type="submit" class="button-exclusive btn btn-outline pull-left"><span>{l s='I confirm my order' mod='cashondelivery'}</span></button>
    </p>
</form>
