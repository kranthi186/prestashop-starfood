 ...
 */
class NewsletterPro_Swift_Signers_SMimeSigner implements NewsletterPro_Swift_Signers_BodySigner
{
    ...
    /**
     * @var NewsletterPro_Swift_StreamFilters_StringReplacementFilterFactory
     */
    protected $replacementFactory;

    /**
     * @var NewsletterPro_Swift_Mime_HeaderFactory
     */
    ...
    public function __construct($signCertificate = null, $signPrivateKey = null, $encryptCertificate = null)
    {
        ...
        $this->replacementFactory = NewsletterPro_Swift_DependencyContainer::getInstance()
    ...
    /**
     * Returns an new NewsletterPro_Swift_Signers_SMimeSigner instance.
     *
     * @param string $certificate
     * @param string $privateKey
     *
     * @return NewsletterPro_Swift_Signers_SMimeSigner
     */
    ...
    /**
     * Change the NewsletterPro_Swift_Message to apply the signing.
     *
    ...
    public function signMessage(NewsletterPro_Swift_Message $message)
    {
        ...
        $messageStream = new NewsletterPro_Swift_ByteStream_TemporaryFileByteStream();
        $this->toSMimeByteStream($messageStream, $message);
        $message->setEncoder(NewsletterPro_Swift_DependencyContainer::getInstance()->lookup('mime.rawcontentencoder'));
    ...
    /**
     * @param NewsletterPro_Swift_InputByteStream $inputStream
     * @param NewsletterPro_Swift_Message   $mimeEntity
     */
    protected function toSMimeByteStream(NewsletterPro_Swift_InputByteStream $inputStream, NewsletterPro_Swift_Message $message)
    {
        ...
        if (null !== $this->signCertificate && null !== $this->encryptCert) {
            $temporaryStream = new NewsletterPro_Swift_ByteStream_TemporaryFileByteStream();
     ...
     */
    protected function createMessage(NewsletterPro_Swift_Message $message)
    {
        $mimeEntity = new NewsletterPro_Swift_Message('', $message->getBody(), $message->getContentType(), $message->getCharset());
    ...
    /**
     * @param NewsletterPro_Swift_FileStream      $outputStream
     * @param NewsletterPro_Swift_InputByteStream $inputStream
     *
     * @throws NewsletterPro_Swift_IoException
     */
    protected function messageStreamToSignedByteStream(NewsletterPro_Swift_FileStream $outputStream, NewsletterPro_Swift_InputByteStream $inputStream)
    {
        $signedMessageStream = new NewsletterPro_Swift_ByteStream_TemporaryFileByteStream();
        ...
        if (!openssl_pkcs7_sign($outputStream->getPath(), $signedMessageStream->getPath(), $this->signCertificate, $this->signPrivateKey, array(), $this->signOptions)) {
            throw new NewsletterPro_Swift_IoException(sprintf('Failed to sign S/Mime message. Error: "%s".', openssl_error_string()));
        }
     ...
     * @throws NewsletterPro_Swift_IoException
     */
    protected function messageStreamToEncryptedByteStream(NewsletterPro_Swift_FileStream $outputStream, NewsletterPro_Swift_InputByteStream $is)
    {
        $encryptedMessageStream = new NewsletterPro_Swift_ByteStream_TemporaryFileByteStream();

        if (!openssl_pkcs7_encrypt($outputStream->getPath(), $encryptedMessageStream->getPath(), $this->encryptCert, array(), 0, $this->encryptCipher)) {
            throw new NewsletterPro_Swift_IoException(sprintf('Failed to encrypt S/Mime message. Error: "%s".', openssl_error_string()));
        }
    ...
    /**
     * @param NewsletterPro_Swift_OutputByteStream $fromStream
     * @param NewsletterPro_Swift_InputByteStream  $toStream
     */
    protected function copyFromOpenSSLOutput(NewsletterPro_Swift_OutputByteStream $fromStream, NewsletterPro_Swift_InputByteStream $toStream)
    {
        $bufferLength = 4096;
        $filteredStream = new NewsletterPro_Swift_ByteStream_TemporaryFileByteStream();
    ...
    /**
     * Merges an OutputByteStream to NewsletterPro_Swift_Message.
     *
     * @param NewsletterPro_Swift_OutputByteStream $fromStream
     * @param NewsletterPro_Swift_Message          $message
     */
    protected function streamToMime(NewsletterPro_Swift_OutputByteStream $fromStream, NewsletterPro_Swift_Message $message)
    {
        $bufferLength = 78;
        $headerData = '';
        ...
        if ('multipart/signed;' === substr($headers['content-type'], 0, 17)) {
            if (!preg_match('/boundary=("[^"]+"|(?:[^\s]+|$))/is', $headers['content-type'], $contentTypeData)) {
                throw new NewsletterPro_Swift_SwiftException('Failed to find Boundary parameter');
            }
            ...
            if (null === $this->headerFactory) {
                $this->headerFactory = NewsletterPro_Swift_DependencyContainer::getInstance()->lookup('mime.headerfactory');
            }
