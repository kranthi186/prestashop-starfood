					...
					{foreach $fields_display AS $key => $params}
						<th {if isset($params.align)} class="{$params.align}" {/if}>
							...
							{else}
								...
								{/if}
                                                                {if $params.filter_key=='sa!quantity'}
                                                                    <div style="float:left;margin-top:35px;"><span class="label-tooltip" data-toggle="tooltip" title="{l s='Use Ctrl+click to select multiple'}">{l s='Show sizes that have stock'}&nbsp;</span></div>
                                                                    <select name="plmSizesFilter[]" multiple="1" style="width:100px;height:75px;">
                                                                        {foreach $plmSizesList as $sizeId=>$sizeName}
                                                                            <option value="{$sizeId}" {if in_array($sizeId, $plmSelectedSizes)}selected="selected"{/if}>{$sizeName}</option>
                                                                        {/foreach}    
                                                                    </select>
                                                                {/if}
							{/if}
					...
					{if $has_actions || $show_filters}
						<th class="actions">
							...
							<span class="pull-right">
								...
								{if $filters_has_value || count($plmSelectedSizes)}
