...
class MYTEXTBLOCK extends Module
{
	public function __construct()
	{
		$this->name = 'mytextblock';
		$this->description = 'mytextblock';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Mytextblock_MONOLANGUAGE';
		$this->text_editor_content = 'Mytextblock_TEXTEDITOR';
		$this->content_wrapper = 'Mytextblock_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Mytextblock_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Mytextblock_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->registerHook('MyTextBlockHook');
		parent::__construct();

		$this->displayName = $this->l('mytextblock');
		$this->description = $this->l('mytextblock');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !MYTEXTBLOCKModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = MYTEXTBLOCKModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = MYTEXTBLOCKModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					MYTEXTBLOCKModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					MYTEXTBLOCKModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																MYTEXTBLOCKModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class MYTEXTBLOCKModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( MYTEXTBLOCKModel::createFilesTable()
				&& MYTEXTBLOCKModel::createContentTable());
	}
