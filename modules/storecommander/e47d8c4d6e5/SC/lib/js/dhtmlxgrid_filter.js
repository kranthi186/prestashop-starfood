...
dhtmlXGridObject.prototype._in_header_select_filter_strict=function(t,i){
	...
	t.firstChild._filter=function(){ 
		var val = t.firstChild.value;
		var combo = self.combos[i];
		if (combo) {
			var ind=combo.values._dhx_find(val);
			val=(ind==-1)?val:combo.keys[ind].toString();
		}
		return function(v){
			if (val=="") return true;
			return (v.toString().toLowerCase()==val.toLowerCase()); 
		}
