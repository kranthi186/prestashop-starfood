...
{
	$sql = "UPDATE "._DB_PREFIX_."category_lang SET description='".pSQL($_POST["description"],1)."' WHERE id_category=".(int)$id_category." AND id_lang=".(int)$id_lang." ";
	if(SCMS)
...
<?php if(version_compare(_PS_VERSION_, '1.5.6.0', '>=')) { ?>
	...
	<?php if(_s("APP_RICH_EDITOR")==1) { ?>
		...
		$().ready(function() {
		    ...
		    <?php echo (_s('CMS_PROPERTIES_DESCRIPTION_CSS')?'content_css : pathCSS+"global.css",':''); ?>
				width: "100%",
				height: "150px",
	...
	<?php } else { ?>
		...
		$(document).ready(function(){
			...
			<?php if(_s('CMS_PROPERTIES_DESCRIPTION_CSS')) { ?>CKEDITOR.config.contentsCss = pathCSS+"global.css" ;<?php } ?>
		});
		</script>
