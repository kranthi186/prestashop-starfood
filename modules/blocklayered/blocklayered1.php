...
class BlockLayered extends Module
{
	...
	private $page = 1;
	protected $searchMode;  // in case if block called from search this variable becomes true
	...
	public function __construct()
	{
        ...
        //echo Configuration::get('AJAX_FILTER_COLOR_CATEGORY_ID');exit;
		...
		if (isset($_REQUEST['search_query']))
		{
			...
			if (!is_array($_REQUEST['search_query']))
			{
				$this->searchQuery = $query = Tools::replaceAccentedChars(urldecode($_REQUEST['search_query']));
			}
		}
	}
	...
	public function hookHeader($params)
	{
		...
		if (!$this->searchMode && Tools::getValue('id_category', Tools::getValue('id_category_layered', Configuration::get('PS_HOME_CATEGORY'))) == Configuration::get('PS_HOME_CATEGORY'))
			return;
			
		// meta info generation only in category mode
		if (! $this->searchMode) 
		{
			...
			if (! empty ( $title ))
				$this->context->smarty->assign ( 'meta_title', $title . ' - ' . Configuration::get ( 'PS_SHOP_NAME' ) );
			else
				$this->context->smarty->assign ( 'meta_title', $category_metas ['meta_title'] );
			
			$this->context->smarty->assign ( 'meta_description', $category_metas ['meta_description'] );
			
			$keywords = substr ( strtolower ( $keywords ), 0, 1000 );
			if (! empty ( $keywords ))
				$this->context->smarty->assign ( 'meta_keywords', rtrim ( $category_title . ', ' . $keywords . ', ' . $category_metas ['meta_keywords'], ', ' ) );
		}

		$this->context->controller->addJS(($this->_path).'jquery.uniform.min.js');
        $this->context->controller->addJS(($this->_path).'blocklayered.js', false);
        $this->context->controller->addCSS(_PS_CSS_DIR_.'uniform.default.min.css');
		...
		if (version_compare(_PS_VERSION_, '1.6.0', '>=') === true)
			$this->context->controller->addCSS(($this->_path).'blocklayered.css', 'all', null, false);
		else
	...
	public function getContent()
	{
		...
		$attribute_groups = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE agl.id_lang = '.(int)$this->context->cookie->id_lang.'
		...
		$features = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE (fv.custom IS NULL OR fv.custom = 0) AND fl.id_lang = '.(int)$this->context->cookie->id_lang.'
	...
	private function getSelectedFilters()
	{
		...
		if (strpos($_SERVER['SCRIPT_FILENAME'], 'blocklayered-ajax.php') === false || Tools::getValue('selected_filters') !== false)
		{
			...
			$selected_filters = array('category' => array(), 'manufacturer' => array(), 'quantity' => array(), 'condition' => array(), 
				'cat_colors'=>array(), 'id_attribute_group'=>array());
			if (!empty($url_attributes))
			{
				foreach ($url_attributes as $url_attribute)
				{
					...
					else if (in_array($attribute_name, array('price', 'weight', 'colors')))
						$selected_filters[$attribute_name] = array($this->filterVar($url_parameters[0]), $this->filterVar($url_parameters[1]));
					else if($attribute_name==$this->l('color'))
					{
						// collect all selected colors, prepare for search categories
						array_walk($url_parameters, function(&$item){
							$item = addslashes($this->filterVar($item));
						});
						// reading category ids corresponding with color names in url 
						$selected_filters['cat_colors'] = array();
						$categoryIds = Db::getInstance()->executeS('select c.id_category from '._DB_PREFIX_.'category c inner join '.
								_DB_PREFIX_.'category_lang cl on cl.id_category=c.id_category and cl.id_lang='.Context::getContext()->cookie->id_lang.
								' and c.id_parent='.Configuration::get('AJAX_FILTER_COLOR_CATEGORY_ID').' where cl.name in(\''.
								implode('\',\'', $url_parameters).'\')');
						foreach($categoryIds as $categoryId)
						{
							$selected_filters['cat_colors'][]=$categoryId['id_category'];
						}
					}
					else
					{
			...
			if (substr($key, 0, 8) == 'layered_')
			{
				...
				if (isset($res[1]))
				{
					...
					else if (in_array($res[1], array('id_attribute_group', 'id_feature')))
					{
						...
						$selected_filters[$res[1]] = $tmp_tab;
					else if ($res[1] == 'cat_colors')
					{
						$selected_filters['cat_colors'][]= intval($value); 
					}
				}
			}
		return $selected_filters;
	...
	/**
	 * Reads products from database based on filters and request parameters. Reads products only for current page
	 * @param unknown $selected_filters array with selected filters (result of getSelectedFilters call)
	 * @return array with products for current page
	 */
	public function getProductByFilters($selected_filters = array())
	{
		...
		foreach ($selected_filters as $key => $filter_values)
		{
			...
			switch ($key)
			{
				...
				break;
				case 'cat_colors':
					//$query_filters_from .= 'LEFT JOIN '._DB_PREFIX_.'category_product col_cat on col_cat.id_category= AJAX_FILTER_COLOR_CATEGORY_ID';
					// $filter_values already converted to int in get selectedFilters method
					$query_filters_where .= ' and p.id_product in (select ccp.id_product from '._DB_PREFIX_.'category_product ccp '.
					'inner join '._DB_PREFIX_.'category cc on ccp.id_category=cc.id_category and cc.id_parent='.
					Configuration::get('AJAX_FILTER_COLOR_CATEGORY_ID').' where ccp.id_category in ('.implode(',', $filter_values).'))';
					break;
			}
		}
	 ...
	 * @return array with products for current page
	 */
	public function getProductByFiltersSearch($selected_filters = array())
	{
		...
		foreach ($selected_filters as $key => $filter_values)
		{
			...
			switch ($key)
			{
				...
				case 'cat_colors':
					//$query_filters_from .= 'LEFT JOIN '._DB_PREFIX_.'category_product col_cat on col_cat.id_category= AJAX_FILTER_COLOR_CATEGORY_ID';
					// $filter_values already converted to int in getSelectedFilters method
					$query_filters_where .= ' and p.id_product in (select ccp.id_product from '._DB_PREFIX_.'category_product ccp '.
							'inner join '._DB_PREFIX_.'category cc on ccp.id_category=cc.id_category and cc.id_parent='.
							Configuration::get('AJAX_FILTER_COLOR_CATEGORY_ID').' where ccp.id_category in ('.implode(',', $filter_values).'))';
							break;
		...
		// create tmp table that contains all product ids after filtration
		Db::getInstance()->execute('DROP TEMPORARY TABLE IF EXISTS '._DB_PREFIX_.'cat_filter_restriction', false);
		
		//SELECT DISTINCT p.id_product, p.id_manufacturer, product_shop.condition, p.weight
		$sql = 	'CREATE TEMPORARY TABLE '._DB_PREFIX_.'cat_filter_restriction ENGINE=MEMORY
							SELECT cp.id_product, MIN(cp.position) position, p.id_manufacturer, product_shop.condition, p.weight FROM '
							._DB_PREFIX_.'category_product cp
							inner JOIN '. _DB_PREFIX_ . 'cat_restriction p USING (id_product)
							'.$price_filter_query_in.$query_filters_from.' WHERE 1 ';
		if (!empty($selected_filters['category']))
		{
			$categories = array_map('intval', $selected_filters['category']);
			$sql .= ' and cp.`id_category` IN ('.implode(',', $categories).') '; 
		}
		
		$sql .= $query_filters_where.' GROUP BY cp.id_product ORDER BY position, id_product';
	...
	// calculates filter totals in category mode
	protected function &filterBlockCalculateTotalsCategory(&$filters, $filter, &$parent, $id_lang)
	{
		$alias = 'product_shop';
		...
		switch ($filter['type'])
		{
			case 'price':
				...
				$sql_query['join'] = 'INNER JOIN `'._DB_PREFIX_.'layered_price_index` psi
								ON (psi.id_product = p.id_product AND psi.id_currency = '.(int)$this->context->currency->id.' AND psi.id_shop='.(int)$this->context->shop->id.')';
			...
			case 'id_attribute_group':// attribute group
				...
				$sql_query['where'] .= ' AND lpa.`id_shop` = '.(int)$this->context->shop->id;
				$sql_query['group'] = '
				...
				if (!Configuration::get('PS_LAYERED_HIDE_0_VALUES'))
				{
					$sql_query['second_query'] = '
							...
							AND lpa.`id_shop` = '.(int)$this->context->shop->id.'
							GROUP BY lpa.id_attribute
			...
			case 'category':
				if (Group::isFeatureActive())
					...
					FROM '._DB_PREFIX_.'category c
					LEFT JOIN '._DB_PREFIX_.'category_lang cl ON (cl.id_category = c.id_category AND cl.`id_shop` = '.(int)$this->context->shop->id.' and cl.id_lang = '.(int)$id_lang.') ';
					...
					{
						// not tested query
						$sql_query['group'] .= 'WHERE c.active = 1 GROUP BY c.id_category ORDER BY c.nleft, c.position';
					}
					else 
					{
						$sql_query['group'] .= 'WHERE c.nleft > '.(int)$parent->nleft.'	AND c.nright < '.(int)$parent->nright.
							($depth ? ' AND c.level_depth <= '.($parent->level_depth+(int)$depth) : '').' AND c.active = 1 '.
	...
	/**
	 * Fills temporary restriction table with found by searchQuery product ids and some some other data. Filters are not applied. 
	 * The table is used to generate filter values. 
	 * @param unknown $id_lang
	 */
	protected function searchFillRestrictionTable($id_lang)
    {
        $context = Context::getContext();
        ...
        $score_array = array();
        $words = explode(' ', Search::sanitize($this->searchQuery, $id_lang, false, $context->language->iso_code));
        ...
        // insert products in temporary table
        $db->execute ( 'DROP TEMPORARY TABLE IF EXISTS ' . _DB_PREFIX_ . 'cat_restriction', false );
        $db->execute ( 'CREATE TEMPORARY TABLE ' . _DB_PREFIX_ . 'cat_restriction ENGINE=MEMORY
				SELECT DISTINCT p.id_product, p.id_manufacturer, product_shop.condition, p.weight FROM '._DB_PREFIX_.'product p INNER JOIN '.
        		_DB_PREFIX_.'product_shop product_shop ON (product_shop.id_product = p.id_product AND product_shop.id_shop = ' .
        		(int) $context->shop->id . ') WHERE p.id_product '.$product_pool.' and product_shop.`active` = 1 AND product_shop.`visibility` IN ("both", "catalog")', false );
        	
        $db->execute ( 'ALTER TABLE ' . _DB_PREFIX_ . 'cat_restriction ADD PRIMARY KEY (id_product),
	...
	/**
	 * Prepares all data for filters block generation, including calcul
	 * @param unknown $selected_filters array, result of getSelectedFilters call 
	 * @return array with various data purposed to assign in smarty and generate filters block
	 */
	public function getFilterBlock($selected_filters = array())
	{
		...
		if ($this->searchMode)
		{
			// create temporary table, that contains all found product ids
			$this->searchFillRestrictionTable($id_lang);
			$parent = null;
			$id_parent = 0;
			
			// filters list is predefined
			$filters = array(array('type'=>'quantity', 'id_value'=>null, 'filter_show_limit'=>0, 'filter_type'=>0),
					array('type'=>'cat_colors', 'id_value'=>null, 'filter_show_limit'=>0, 'filter_type'=>0),
					array('type'=>'id_attribute_group', 'id_value'=>1, 'filter_show_limit'=>0, 'filter_type'=>0),
				);
		} 
		else 
		{
		...
		{
			// calculate totals
			$products = $this->filterBlockCalculateTotalsCategory($filters, $filter, $parent, $id_lang);
			...
			switch ($filter['type'])
			{
				...
				case 'category':
					...
					break;
				case 'cat_colors':
					// reading color categories
					$colorCategories = Db::getInstance()->executeS('select c.id_category, name, color_code from '._DB_PREFIX_.
					 'category c inner join '._DB_PREFIX_.'category_lang cl on cl.id_category=c.id_category and id_lang='.$id_lang.
					 ' inner join '._DB_PREFIX_.'category_shop cs on cs.id_category=c.id_category and cs.id_shop='.$id_shop
                                         .' where c.id_parent='.Configuration::get('AJAX_FILTER_COLOR_CATEGORY_ID').' order by cs.position');
					
					// generate options
					$colorOptions = array();
					foreach($colorCategories as $colCategory)
					{
						$item = array('color'=>$colCategory['color_code'], 'name' => $colCategory['name'], 
								'nbr' => 1, // number of such products
					       		'url_name' => strtolower($colCategory['name']),
						);
						if (in_array($colCategory['id_category'], $selected_filters['cat_colors']))
							 //|| in_array(strtolower($colCategory['name']), $selected_filters['cat_colors'] ))
						{
							$item['checked'] = true;
						} 
						$colorOptions [$colCategory['id_category']]= $item;
								//'link' => 'http://dmitri.wheel/grosshandel16/en/12-abendkleider#cat_color-beige');
					}
					$filter_blocks[] = array (
										'type_lite' => 'cat_colors',
										'type' => 'cat_colors',
										'id_key' => 0, 'name' => $this->l('Color'),
								        'is_color_group' => true,
										'values' => $colorOptions,
										'filter_show_limit' => $filter['filter_show_limit'],
										'filter_type' => $filter['filter_type']
		...
		if ($this->searchMode)
		{
			$baseUrl = $this->context->link->getPageLink('search').'?search_query='.urlencode($this->searchQuery);		
		}
		else 
		{
			$baseUrl = Context::getContext()->link->getCategoryLink($parent, null, null);
			//echo $currentSEOUrlWOPage;
		}
		$currentSEOUrlWOPage = $baseUrl.'#'.$param_selected;
		...
		foreach ($filter_blocks as &$type_filter)
		{
			...
			if (count($type_filter) > 0 && !isset($type_filter['slider']))
			{
				foreach ($type_filter['values'] as $key => $values)
				{
					...
					$type_filter['values'][$key]['link'] = $baseUrl.'#'.ltrim($parameters, '/');
		...
		$cache = array(
			...
			'currentSEOUrlWOPage' => $currentSEOUrlWOPage
		);
	...
	public function ajaxCall()
	{
		...
		$category = new Category(Tools::getValue('id_category_layered', Configuration::get('PS_HOME_CATEGORY')), (int)$this->context->cookie->id_lang);
		...
		$this->context->smarty->assign(
			array(
				...
				'seoUrls' => true, 
				'current_url'=>$filter_block['currentSEOUrlWOPage']
			)
		);
		...
		if (version_compare(_PS_VERSION_, '1.6.0', '>=') === true)
			$vars = array_merge($vars, array('pagination_bottom' => $this->context->smarty->assign('paginationId', 'bottom')
	...
	/**
	 * Method called by ajax from search results page. Returns json that contains all variables to update page 
	 * @return json to be returned to calling script
	 */
	public function ajaxCallSearch()
	{
		$selected_filters = $this->getSelectedFilters();
		$filter_block = $this->getFilterBlock($selected_filters);
		$this->getProductsSearch($selected_filters, $products, $nb_products, $p, $n, $pages_nb, $start, $stop, $range);
	...
	/**
	 * Reads products for search page. Applies filters to products read in getFilterBlock method (uses result of sql query)
	 * Returns array with products and pagination data in parameters passed by reference.
	 * @param unknown $selected_filters
	 * @param unknown $products
	 * @param unknown $nb_products
	 * @param unknown $p
	 * @param unknown $n
	 * @param unknown $pages_nb
	 * @param unknown $start
	 * @param unknown $stop
	 * @param unknown $range
	 */
	public function getProductsSearch($selected_filters, &$products, &$nb_products, &$p, &$n, &$pages_nb, &$start, &$stop, &$range)
	{
		$products = $this->getProductByFiltersSearch($selected_filters);
		$products = Product::getProductsProperties((int)$this->context->cookie->id_lang, $products);
	
		// preselected size if it is set
		if (count($selected_filters['id_attribute_group']))
		{
			list($groupId, $optionId) = explode('_', reset($selected_filters['id_attribute_group']));
			foreach($products as &$product)
			{
				$product['link'] = $product['link'].'?group_'.$groupId.'='.$optionId;
			}
		}
	...
	public function rebuildLayeredCache($products_ids = array(), $categories_ids = array())
	{
		...
		while ($product = $db->nextRow($result))
		{
			...
			foreach ($c as $id_category => $category)
			{
				...
				if (!isset($done_categories[(int)$id_category]['col']))
				{
					$filter_data['layered_selection_cat_colors_filter'] = array('filter_type' => 0, 'filter_show_limit' => 0);
					$done_categories[(int)$id_category]['col'] = true;
					$to_insert = true;
				}
			}
		}
					...
					if (!in_array($id_category, $categories[$id_shop])) // Last definition, erase preivious categories defined
					{
						...
						foreach ($data as $key => $value)
							if (substr($key, 0, 17) == 'layered_selection')
							{
								...
								else if ($key == 'layered_selection_cat_colors_filter')
									$sql_to_insert .= '('.(int)$id_category.', '.(int)$id_shop.', NULL,\'cat_colors\','.(int)$n.', '.(int)$limit.', '.(int)$type.'),';
