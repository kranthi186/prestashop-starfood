...
class MYTEXTBLOCK2 extends Module
{
	public function __construct()
	{
		$this->name = 'mytextblock2';
		$this->description = 'mytextblock2';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Mytextblock2_MONOLANGUAGE';
		$this->text_editor_content = 'Mytextblock2_TEXTEDITOR';
		$this->content_wrapper = 'Mytextblock2_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Mytextblock2_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Mytextblock2_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('mytextblock2');
		$this->description = $this->l('mytextblock2');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !MYTEXTBLOCK2Model::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = MYTEXTBLOCK2Model::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = MYTEXTBLOCK2Model::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					MYTEXTBLOCK2Model::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					MYTEXTBLOCK2Model::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																MYTEXTBLOCK2Model::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class MYTEXTBLOCK2Model extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( MYTEXTBLOCK2Model::createFilesTable()
				&& MYTEXTBLOCK2Model::createContentTable());
	}
