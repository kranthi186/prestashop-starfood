...
class LeoproductsearchproductsearchModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        if ($this->instant_search && !is_array($query)) {
            ...
            foreach($search['result'] as $key => $product){
                if($this->context->language->id==1){
                    ...
                    $search['result'][$key]['name_original'] = $ori_name;
                }
                else{
                    $prod = new Product($product['id_product']);
            ...
            for($i = 0; $i < count( $search['result']) ; $i++){
                if($this->context->language->id==1){
                    $prod = new Product($search['result'][$i]['id_product']);
                    $ori_name = $prod->name['2'];
                    $search['result'][$i]['name_original'] = $ori_name;
                }
                else{
                    $prod = new Product($search['result'][$i]['id_product']);
                    $ori_name = $prod->name['1'];
            ...
            foreach($result as $key => $product){
                if($this->context->language->id==1){
                    $prod = new Product($product['id_product']);
                    $ori_name = $prod->name['2'];
                    $result[$key]['name_original'] = $ori_name;
                }
                else{
                    $prod = new Product($product['id_product']);
                    $ori_name = $prod->name['1'];
        ...
        $this->context->smarty->assign(array(
            ...
            'request' => Tools::getHttpHost(true, true).$requestParts['path'] .'?'. http_build_query($reqParams)
        ));
