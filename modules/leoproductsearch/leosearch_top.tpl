...
<div id="leo_search_block_top" class="col-lg-7 col-md-6 col-sm-8 col-xs-12 exclusive">
	<form method="get" action="{$link->getPageLink('search', true)|escape:'html':'UTF-8'}" id="leosearchtopbox">
			...
			<div class="category_filter">	
				<select class="col-md-3 col-sm-4 col-xs-6" name="cate" id="cate">
					<option value="">{l s='All Categories' mod='leoproductsearch'}</option>
					 ...
					 <option class="option_search_{$cate.id_category|escape:'htmlall':'UTF-8'|stripslashes}" value="{$cate.id_category|escape:'htmlall':'UTF-8'|stripslashes}" {if isset($selectedCate) && $cate.id_category eq $selectedCate}selected{/if} >{$cate.name}</option>
					 {/foreach}
				</select>
