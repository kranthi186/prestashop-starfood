...
class catalog extends SCPDFCatalogCreator
{
    ...
    public function createCatalog($format, $start, $limit, $orderBy, $orderWay)
    {
        ...
        $posx = array(27, 122, 27, 122, 27, 122);
        $posy = array(40, 40, 115, 115, 190, 190);
        $idx = 0;
            ...
            foreach ($products AS $productrow)
            {
                ...
                if (Validate::isLoadedObject($product))
                {
                    ...
                    if ($idx > 1)
                    {
                        if (($idx % 2) == 0)
                        {
                            $newy = ceil(max(array($this->last_y1, $this->last_y2)) + 10);
                            $table = false;
                        }
                    }
                    else
                    ...
                    if ($idx == 0 || $idx == 2 || $idx == 4)
                        $actual_pos_y = $this->GetY() + 10;

                    $this->addProductBox($product, $productrow['combinations'], $image_width, $image_height, $posx[$idx], $actual_pos_y/* $posy[$idx] */, 'small', $newy, $idx);
                    $idx++;
                } else
                {
                    die('Invalid product');
    ...
    public function addProductBox($product, $productCombinations, $width, $height, $posx, $posy, $theme, $newy, $idx)
    {
        ...
        if ($theme == 'big')
        {
            ...
            $this->MultiCell($width, 3, html_entity_decode(strip_tags($product_label . "\n" . $this->mod->tt('Ref.') . ' ' . $product->reference), ENT_NOQUOTES, $this->encoding()), 0, 'C', 0);
        ...
        }else
        {
            ...
            if ($this->vatexc == 1) :
                $chaineHTML.="<br/>" . $this->mod->tt('Price Exc. Tax') . str_repeat('.', 30 - Tools::strlen(trim($this->mod->tt('Price Exc. Tax'))));
                $chaineHTML.=$this->getPrice($product->id, 0, 0);
            endif;

            if ($this->vatinc == 1) :
                $chaineHTML.="<br/>" . $this->mod->tt('Price Inc. Tax') . str_repeat('.', 30 - Tools::strlen(trim($this->mod->tt('Price Inc. Tax'))));
