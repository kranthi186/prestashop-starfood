...
class catalog extends SCPDFCatalogCreator {
    ...
    public function addLibelleLine() {
        ...
        $posx = array(10, 30, 180, 200, 220);
        $widths = array(20, 150, 20, 20, 20);
        $this->SetFillColor(240, 240, 240);
        $this->SetTextColor(0, 0, 0);
        $this->SetFont($this->fontname(), 'B', 8);
        $this->Ln(7);
    ...
    public function _addLineWithLayout($infos, $product, $is_combination=false, $has_combination=false)
    {
    	...
    	$posx = array(10, 34, 50, 180, 200);
    	$widths = array(20, 20, 130, 20, 20);
    	$this->SetFillColor(240, 240, 240);
    	$this->SetTextColor(0, 0, 0);
    	...
    	if (!$has_combination || $is_combination) {
                ...
                if ($this->vatinc == 1) {
                    ...
                    $this->SetX($posCell);
                   $this->Cell($widths[3], $height_line,  $infos["vatinc"], 1, 0, 'R', 0, '', 1);
                }  
    	}
    	else
    	{
    		if ($this->vatexc == 1 && $this->vatinc == 1)
    			$this->Cell($widths[3]+$widths[3], $height_line, "", 1, 0, 'R', 1);
