...
class catalog extends SCPDFCatalogCreator {
    ...
    public function addLibelleLine() {
        ...
        $this->setX($posx[1]);
        $this->Cell($widths[1] - $this->offset, 6, $this->mod->tt('Product'), 1, 0, 'L', 1);
        $this->setX($posx[2] - $this->offset);
        $this->Cell($widths[2], 6, $this->mod->tt('UPC'), 1, 0, 'C', 0);
	...
	public function _addLineWithLayout($infos, $product, $is_combination=false, $has_combination=false)
	{
		...
		if($height_line<16 && !empty($infos["upc"]))
			$height_line = 16;
		...
		//$this->setX($posx[2]-$this->offset);
		if(!empty($infos["upc"]))
			$this->UPC_A($posx[2] - $this->offset, $this->start_y, $infos["upc"], 5);
