...
class catalog extends SCPDFCatalogCreator {
            ...
            foreach ($products AS $productrow) {
                ...
                if (!isset($this->categ_names[$id_category])) {
                  /*  if (count($this->categ_names) > 0)
                    ...
                    $this->addCategoryLinePhoto($category->getName($this->id_lang), ($category->id_parent != 1 ? $this->categ_names[$category->id_parent] . ' > ' : ''), 'small');
                }
    ...
    public function addCategoryLinePhoto($categname, $prefix = "", $theme = 'big') {
        /*$this->addLibelleLine($theme);
        $this->SetFillColor(200, 200, 200);
        $this->SetTextColor(0, 0, 0);
        $this->SetFont($this->fontname(), 'BI', 10);
        $this->Ln(8);
        //adjusts the category title according the vatinc and the vatexc
        if ($this->vatexc == 1 && $this->vatinc == 1)
            $this->Cell(180, 8, $prefix . parent::hideCategoryPosition($categname), 1, 0, 'L', 0);
        else
            $this->Cell(180, 8, $prefix . parent::hideCategoryPosition($categname), 1, 0, 'L', 0);*/
    ...
    public function addLibelleLine()
	{
		...
		$this->Cell($widths[2], 6, $this->mod->tt('Weight'), 1, 0, 'C', 0);
	...
	public function _addLineWithLayout($infos, $product, $is_combination=false, $has_combination=false)
	{
    	...
    	//if ($this->getY() > (297 - 30))
    	if ($this->getY() > (280- 30)) {
			//-26 -> to prevent the pb of the multicell that flows over the first combination
			$this->Ln(14);
			//$this->AddPage();
		}
		...
		if(!$is_combination && $this->conf['PS_SC_PDFCATALOG_LINKS'])
			$this->writeHTML('<table border="0.5" cellpadding="5" cellspacing="0"><tr><td width="2cm"></td><td width="'.(($widths[1]-$this->offset)/10).'cm"><a href="'.$infos["link"].'" style="color: #000000; text-decoration: none;">'.$product_label.'</a></td></tr></table>', true, false, false, false, '');
		else
			$this->writeHTML('<table border="0.5" cellpadding="5" cellspacing="0"><tr><td width="2cm"></td><td width="'.(($widths[1]-$this->offset)/10).'cm">'.$product_label.'</td></tr></table>', true, false, false, false, '');
		...
		// Insertion colone Poids
		$this->SetXY($posx[2]-$this->offset, $this->start_y);
		$this->Cell($widths[2], $height_line, $infos["weight"], 1, 0, 'C', 0);
		...
		if (!$has_combination || $is_combination) {
			if ($this->vatexc == 1) {
				$this->Cell($widths[3], $height_line, $infos["vatexc"], 1, 0, 'R', 0, '', 1);
			}
    	...
    	{
    		if ($this->vatexc == 1 && $this->vatinc == 1)
    			$this->Cell($widths[3]+$widths[4], $height_line, "", 1, 0, 'R', 1);
