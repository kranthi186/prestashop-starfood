...
$(document).ready(function(){
	...
	if($("#reference_number_format").val().indexOf("{COUNTER}") > -1){
		...
		if($(".CounterReferenceLength input:checked").val() == 1){displayFields("#counter_reference_length",true);}else{displayFields("#counter_reference_length",false);}
		if($(".ResetCounterInterval input:checked").val() == 3){displayFields("#counter_reset_number",true);}else{displayFields("#counter_reset_number",false);}
		if($(".ResetCounterInterval input:checked").val() == 4){displayFields("#counter_reset_date",true);}else{displayFields("#counter_reset_date",false);}
		if($(".CounterReferenceLength input:checked").val() == 1){
	...
	$(".CounterReferenceLength input").click(function(){
	...
	$(".ResetCounterInterval input").click(function(){
	...
	$(".customreferencenumbers").on("submit", function(){
		...
		if($("#reference_number_format").val().indexOf("{COUNTER}") > -1 && ($(".ResetCounterInterval input:checked").val() == 3) && (!isNumber($("#counter_reset_number").val()) || $("#counter_reset_number").val() == "" || $("#counter_reset_number").val() < 1)){
			if(errorMessage.length > 0){
				errorMessage += ", "
			}
			errorMessage += counterResetNumberError;
		}
		if($("#reference_number_format").val().indexOf("{COUNTER}") > -1 && ($(".ResetCounterInterval input:checked").val() == 4) && ($("#counter_reset_date").val() == "" || $("#counter_reset_date").val() == 0 || (counterResetDate.datepicker( "getDate" ) <= new Date()))){
			if(errorMessage.length > 0){
				errorMessage += ", "
			}
