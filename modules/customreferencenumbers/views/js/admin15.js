...
$(document).ready(function(){
	...
	if($("#reference_number_format").val().indexOf("{COUNTER}") > -1){
		displayFields("#counter_start_at",true);displayFields("#counter_increment_by",true);displayFields(".ResetCounterInterval:eq(0)",true);displayFields(".CounterResetNow:eq(0)",true);displayFields(".CounterReferenceLength:eq(0)",true);
		if($(".CounterReferenceLength").parent().find("input:checked").val() == 1){displayFields("#counter_reference_length",true);}else{displayFields("#counter_reference_length",false);}
		if($(".ResetCounterInterval").parent().find("input:checked").val() == 3){displayFields("#counter_reset_number",true);}else{displayFields("#counter_reset_number",false);}
		if($(".ResetCounterInterval").parent().find("input:checked").val() == 4){displayFields("#counter_reset_date",true);}else{displayFields("#counter_reset_date",false);}
		if($(".CounterReferenceLength").parent().find("input:checked").val() == 1){
	...
	$(".CounterReferenceLength").parent().find("input").click(function(){
	...
	$(".ResetCounterInterval").parent().find("input").click(function(){
	...
	$("#counter_reference_length").bind("keyup change", function(){
		if(counterActualValue != ""){
			...
			$("#counter_start_at").parent().find(".help-block").remove();
			$("#counter_start_at").parent().append("<p class=\'help-block\'>Next counter value: " + nextCounterValue + "</p>");
		}
	});
	$(".customreferencenumbers").on("submit", function(){
		...
		if($("#reference_number_format").val().indexOf("{COUNTER}") > -1 && ($(".ResetCounterInterval").parent().find("input:checked").val() == 3) && (!isNumber($("#counter_reset_number").val()) || $("#counter_reset_number").val() == "" || $("#counter_reset_number").val() < 1)){
			if(errorMessage.length > 0){
				errorMessage += ", "
			}
			errorMessage += counterResetNumberError;
		}
		if($("#reference_number_format").val().indexOf("{COUNTER}") > -1 && ($(".ResetCounterInterval").parent().find("input:checked").val() == 4) && ($("#counter_reset_date").val() == "" || $("#counter_reset_date").val() == 0 || (counterResetDate.datepicker( "getDate" ) <= new Date()))){
			if(errorMessage.length > 0){
				errorMessage += ", "
			}
	...
	$( "label.ResetCounterInterval:eq(0)" ).css({ "width": "66px", "margin-right": "30px" });
	$( "label.ResetCounterInterval:eq(1)" ).css({ "width": "73px", "margin-right": "30px" });
	$( "label.ResetCounterInterval:eq(2)" ).css({ "width": "62px", "margin-right": "30px" });
	$( "label.ResetCounterInterval:eq(3)" ).css({ "width": "239px", "margin-right": "30px" });
	$( "label.ResetCounterInterval:eq(4)" ).css({ "width": "170px", "margin-right": "30px" });
	$( "label.ResetCounterInterval:eq(5)" ).css({ "width": "40px", "margin-right": "30px" });
});
function displayFields(identification,isDisplay){
	if(isDisplay){
		$(identification).parent().show();$(identification).parent().prev().show();
	}else{
		$(identification).parent().hide();$(identification).parent().prev().hide();	
	}
}
