...
class HOMEPAGEVIDEO extends Module
{
	public function __construct()
	{
		$this->name = 'homepagevideo';
		$this->description = 'Home Page Video';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Homepagevideo_MONOLANGUAGE';
		$this->text_editor_content = 'Homepagevideo_TEXTEDITOR';
		$this->content_wrapper = 'Homepagevideo_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Homepagevideo_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Homepagevideo_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('Home Page Video');
		$this->description = $this->l('Home Page Video');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !HOMEPAGEVIDEOModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = HOMEPAGEVIDEOModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = HOMEPAGEVIDEOModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					HOMEPAGEVIDEOModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					HOMEPAGEVIDEOModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																HOMEPAGEVIDEOModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class HOMEPAGEVIDEOModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( HOMEPAGEVIDEOModel::createFilesTable()
				&& HOMEPAGEVIDEOModel::createContentTable());
	}
