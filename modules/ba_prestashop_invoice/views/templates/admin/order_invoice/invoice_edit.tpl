					...
					<div class='col-sm-9'>
						<select name="colums_content[]">
							...
							<option value="13">{l s='Product Name with Customization' mod='ba_prestashop_invoice'}</option>
                                                        <option value="-1">{l s='Stock state (in stock)' mod='ba_prestashop_invoice'}</option>
                            ...
                            <option value="19">{l s='Product Number(postion)' mod='ba_prestashop_invoice'}</option>
                            <option value="20">{l s='Pieces per Box' mod='ba_prestashop_invoice'}</option>
                            <option value="21">{l s='Brand' mod='ba_prestashop_invoice'}</option>
                            <option value="22">{l s='Weight' mod='ba_prestashop_invoice'}</option>
						</select>
					</div>
				</div>
				<div class='form-group col-sm-6'>
		...
		<form class="form-horizontal"  method="POST" id="form_template">
			...
			<div class="" style="padding:0 10px;width:100%;float:left;">
				...
				<div class="col-sm-12" style="margin-top:7px;">
					...
					<div class="col-sm-12" style="margin-bottom: 10px;">
						...
						<div class="col-sm-3"><textarea name="descriptionInvoice">{$invoiceTemplate.description|escape:'htmlall':'UTF-8'}</textarea></div>
                                        ...
                                        <div class="col-sm-12" style="margin-bottom: 10px;">
						<div class="col-sm-4 control-label" style="padding:0;font-size: 13px;">{l s='Sum to pay percent' mod='ba_prestashop_invoice'}: </div>
						<div class="col-sm-3"><input type="text" name="sum_to_pay_percent" value="{$invoiceTemplate.sum_to_pay_percent|escape:'htmlall':'UTF-8'}" size="6" style="width: auto; display: inline;" > %
                                                    <div class="help-block">{l s='Initial sum to pay in invoice document will be set based on this percent: invoiceAmount*sumToPayPercent' mod='ba_prestashop_invoice'}</div>
                                                </div>
					</div>
                                        <div class="col-sm-12" style="margin-bottom: 10px;">
						<div class="col-sm-4 control-label" style="padding:0;font-size: 13px;">{l s='Due date +' mod='ba_prestashop_invoice'}: </div>
						<div class="col-sm-3"><input type="text" name="due_date_plus" value="{$invoiceTemplate.due_date_plus|escape:'htmlall':'UTF-8'}" size="6" style="width: auto; display: inline;" >
                                                    <div class="help-block">{l s='Number of days to add to current date to get due date' mod='ba_prestashop_invoice'}</div>
                                                </div>
					</div>
                                        <div class="col-sm-12" style="margin-bottom: 10px;">
						<div class="col-sm-4 control-label" style="padding:0;font-size: 13px;">{l s='Auto set paid' mod='ba_prestashop_invoice'}: </div>
						<div class="col-sm-5"><input type="checkbox" name="auto_set_paid" value="1" {if $invoiceTemplate.auto_set_paid==1}checked=checked{/if}  autocomplete="off" /></div>
					</div>
                                        <div class="col-sm-12" style="margin-bottom: 10px;">
						<div class="col-sm-4 control-label" style="padding:0;font-size: 13px;">{l s='Disable tax' mod='ba_prestashop_invoice'}: </div>
						<div class="col-sm-5"><input type="checkbox" name="no_tax" value="1" {if $invoiceTemplate.no_tax==1}checked=checked{/if}  autocomplete="off" /></div>
					</div>
					<div class="col-sm-12" style="margin-bottom: 10px;">
						...
						<div class="col-sm-5">
							<span class="switch prestashop-switch fixed-width-lg">
								...
								<a class="slide-button btn"></a>
                                        ...
                                        <div class="col-sm-12" style="margin-bottom: 10px;">
						<div class="col-sm-4 control-label" style="padding:0;font-size: 13px;">{l s='Category' mod='ba_prestashop_invoice'}: </div>
						<div class="col-sm-5">
                                                    {foreach from=$templateCatsList item=categoryName key=categoryId}
                                                        <label class="text-nowrap"><input type="checkbox" name="tpl_cat_ids[]" value="{$categoryId}" autocomplete="off" {if in_array($categoryId, $templateCatIds)}checked="checked" {/if}>&nbsp;{$categoryName|replace:' ':'&nbsp;'}&nbsp;&nbsp;</label>
                                                    {/foreach}
						</div>
					</div>
				</div>
				...
				<div class="row list_token col-sm-12" rev="close" style="height:100px;overflow:hidden;margin-bottom: 20px;">
						...
						<label class="control-label">{l s='id of customer (account number)' mod='ba_prestashop_invoice'}</label>
					</div>
                                        <div class="ba_invoice_token_wrapper">
						<label>[orderTotal_XX_te]: </label>
						<label class="control-label">{l s='XX% of order total with tax excluded. XX should be 2 digits.' mod='ba_prestashop_invoice'}</label>
					</div>
                                        <div class="ba_invoice_token_wrapper">
						<label>[orderTotal_XX_ti]: </label>
						<label class="control-label">{l s='XX% of order total with tax included.  XX should be 2 digits.' mod='ba_prestashop_invoice'}</label>
					</div>
                                        <div class="ba_invoice_token_wrapper">
						<label>[orderTotal_XX_VAT]: </label>
						<label class="control-label">{l s='VAT included in XX% of order total with tax included. XX should be 2 digits.' mod='ba_prestashop_invoice'}</label>
					</div>
                                        <div class="ba_invoice_token_wrapper">
						<label>[today+10]: </label>
						<label class="control-label">{l s='Date 10 days in future from today' mod='ba_prestashop_invoice'}</label>
					</div>
                                        <div class="ba_invoice_token_wrapper">
						<label>[orderInvoiceTxt]: </label>
						<label class="control-label">{l s='Text from order details page' mod='ba_prestashop_invoice'}</label>
					</div>
					<div class="ba_invoice_token_wrapper">
				...
				<div class="product_template">
					...
					<div class="number_column">
						<div class="form-group">
							<label class="col-sm-1 control-label">{l s='Numbber of table' mod='ba_prestashop_invoice'}</label>
							<div class="col-sm-10">
								...
								<div class='col-sm-9'>
									<select name="colums_content[]">
										...
										<option value="13" {if $invoiceTemplate.columsContentJson.$i=="13"}selected{/if}>{l s='Product Name with Customization' mod='ba_prestashop_invoice'}</option>
                                                                                <option value="-1" {if $invoiceTemplate.columsContentJson.$i=="-1"}selected{/if}>{l s='Stock state (in stock)' mod='ba_prestashop_invoice'}</option>
                                        ...
                                        <option value="19"  {if $invoiceTemplate.columsContentJson.$i=="19"}selected{/if}>{l s='Product Number(postion)' mod='ba_prestashop_invoice'}</option>
                                        <option value="20" {if $invoiceTemplate.columsContentJson.$i=="20"}selected{/if}>{l s='Pieces per Box' mod='ba_prestashop_invoice'}</option>
                                        <option value="21" {if $invoiceTemplate.columsContentJson.$i=="21"}selected{/if}>{l s='Brand' mod='ba_prestashop_invoice'}</option>
                                        <option value="22" {if $invoiceTemplate.columsContentJson.$i=="22"}selected{/if}>{l s='Weight' mod='ba_prestashop_invoice'}</option>
									</select>
								</div>
							</div>
							<div class='form-group col-sm-6'>
