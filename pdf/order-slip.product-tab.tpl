...
<table class="product" width="100%" cellpadding="4" cellspacing="0">
	...
	<thead>
		<tr>
			<th class="product header small" width="10%">{l s='Photo' pdf='true'}</th>
			<th class="product header small" width="50%">{l s='Product / Reference' pdf='true'}</th>
		...
		{else}
			{foreach $order_details as $order_detail}
				...
				<tr class="product {$bgcolor_class}">
					<td><img src="{$link->getImageLink('a', $order_detail.image->id, 'cart_default')}" /></td>
					<td class="product left">
						{$order_detail.product_name} / {$order_detail.product_supplier_reference}
					</td>
					<td class="product center">
