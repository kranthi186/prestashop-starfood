...
{block name="override_tpl"}
	...
	var txt_confirm = "{l s='Are you sure?' js=1}";
        var aiNoProductsSelected = '{l s='You need to select at least one product in list below' js=1}';
	...
	var order_discount_price = {if ($order->getTaxCalculationMethod() == $smarty.const.PS_TAX_EXC)}
								...
								{/if};
	var order_discount_price_ti = {$order->total_discounts_tax_incl};
	var order_discount_price_te = {$order->total_discounts_tax_excl};
	var orderTotalTi = {$order->total_paid};
	var orderTotalProductsTe = {$order->total_products};
	var errorRefund = "{l s='Error. You cannot refund a negative amount.'}";
        var readMessagesConfirmation = "{l s='There are notes in this order. Did you read them?'}";
        var showReadMessagesConfirmation = {if count($customer_thread_message)}true{else}false{/if};
	</script>
		...
		<div class="col-lg-7">
			<div class="panel">
				...
				<div class="well hidden-print">
					...
					&nbsp;
                                        <a class="btn btn-default" target="_blank" href="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}&amp;submitBulkhtmlShippingInfoorder&orderBox[]={$order->id}">
				...
				<div class="tab-content panel">
					...
					<div class="tab-pane active" id="status">
						...
						<form action="{$currentIndex|escape:'html':'UTF-8'}&amp;vieworder&amp;token={$smarty.get.token}" method="post" class="form-horizontal well hidden-print">
							...
							<div class="row">
								<div class="col-lg-9">
                                                                    <select id="id_order_payment_status" class="chosen form-control" name="id_order_payment_status" autocomplete="off">
                                                                            <option value="0" {if $order->payment_status_id==0} selected="selected" disabled="disabled"{/if}>{l s='Not set'}</option>
									{foreach from=$paymentStatuses item=paymentStatus}
										<option value="{$paymentStatus['id_order_payment_status']|intval}"{if $paymentStatus['id_order_payment_status'] == $order->payment_status_id} selected="selected" disabled="disabled"{/if}>{$paymentStatus['name']|escape}</option>
									{/foreach}
									</select>
									<input type="hidden" name="id_order" value="{$order->id}" />
								</div>
								<div class="col-lg-3">
									<button type="submit" name="submitPaymentStatus" class="btn btn-primary">
										{l s='Update status'}
									</button>
								</div>
							</div>
				...
				<div id="messages" class="well hidden-print">
					<form action="{$smarty.server.REQUEST_URI|escape:'html':'UTF-8'}&amp;token={$smarty.get.token|escape:'html':'UTF-8'}" method="post" onsubmit="if (getE('visibility').checked == true) return confirm('{l s='Do you want to send this message to the customer?'}');">
						<div id="message" class="form-horizontal">
									...
									<p class="help-block">
										<a href="{$link->getAdminLink('AdminOrderMessage')|escape:'html':'UTF-8'}">
											...
											<i class="icon-external-link"></i>
                                                        ...
                                                        <div class="form-group">
								<label class="control-label col-lg-3">{l s='To'}</label>
                                                                <div class="col-lg-9">
                                                                    <input class="form-control" name="to" id="custMsgTo" value="" />
                                                                    <div class="help-block">
                                                                        {l s='Several comma separated emails may be enterred. If some email(s) entered, then message will be sent only to entered email(s). If empty, message will be sent to customer email in db.'}
								    </div>
                                                                </div>
                                                        </div>
                                                        <div class="form-group">
								<label class="control-label col-lg-3">{l s='Email subject'}</label>
                                                                <div class="col-lg-9">
                                                                    <input class="form-control" name="subject" id="custMsgSubject" value="{l s='New message regarding your order'}" />
                                                                </div>
                                                        </div>
								...
								<div class="col-lg-9">
									<span class="switch prestashop-switch fixed-width-lg">
										...
										<a class="slide-button btn"></a>
                                                        ...
                                                        <div class="form-group">
								<label class="control-label col-lg-3">{l s='Attach invoice'}</label>
								<div class="col-lg-9">
                                                                    <select class="form-control" name="attach_invoice_id">
                                                                        <option value="0">--</option>
                                                                    {foreach from=$order->getDocuments() item=document}
                                                                        <option value="{$document->id}">{if get_class($document) eq 'OrderInvoice'}
							...
							{/if} - {dateFormat date=$document->date_add}</option>
                                                                    {/foreach}
                                                                    </select>
                                                                </div>
							</div>
				...
				<div class="panel">
					...
					<div class="table-responsive">
						<table class="table" id="orderProducts">
							<thead>
								<tr>
                                                                    <th><input type="checkbox" name="prodductAll" id="productIdCheckAll" onClick="{literal}if($(this).is(':checked')) {$('.productIdCheckBox').prop('checked', true );} else {$('.productIdCheckBox').prop('checked', false );}{/literal}" /></th>
									<th></th>
									<th><span class="title_box ">{l s='Product'}</span></th>
                                                                        <th><span class="title_box ">{l s='In stock'}</span></th>
                                                                        <th><span class="title_box ">{l s='Shipped'}</span></th>
									<th>
									...
									<th style="display: none;" class="standard_refund_fields">
										...
										{else}
											{l s='Cancel'}
										{/if}
										<input type="checkbox" name="productReturnAll" id="productReturnIdCheckAll" onClick="{literal}var masterChecked = $(this).is('checked'); $('.returnOrderDetailCheckBox').each(function(){if($(this).is('checked')==masterChecked) $(this).click();});{/literal}" autocomplete="off" />
									</th>
					...
					{/if}
						...
						<button id="print_label" class="btn btn-default" type="button" >
							<i class="icon-ticket"></i>
							{l s='Print Labels'}
						</button>
						<button id="print_label_pdf" class="btn btn-default" type="button" >
							<i class="icon-ticket"></i>
							{l s='Print Labels Pdf'}
						</button>
					</div>
					...
					<div style="display: none;" class="standard_refund_fields form-horizontal panel">
						<div class="form-group">
							...
							<p class="checkbox" id="spanShippingBack">
							...
							<p class="radio">
								<label id="lab_refund_total_1" for="refund_total_1">
									<input type="radio" value="0" name="refund_total_voucher_off" id="refund_total_1" />
									{l s='Include amount of initial voucher: '}
								</label>
							</p>
							<p class="radio">
								<label id="lab_refund_total_2" for="refund_total_2">
									<input type="radio" value="1" name="refund_total_voucher_off" id="refund_total_2" checked="checked" />
						...
						<div class="row">
							<input type="submit" name="cancelProduct" {if count($customer_thread_message)}onClick="return confirm(readMessagesConfirmation)"{/if} value="{if $order->hasBeenDelivered()}{l s='Return products'}{elseif $order->hasBeenPaid()}{l s='Refund products'}{else}{l s='Cancel products'}{/if}" class="btn btn-default" />
						</div>
						{/if}
					</div>
					<div style="display:none;" class="partial_refund_fields">
						...
						{/if}
						<br/>
						<button type="submit" name="partialRefund" class="btn btn-default" {if count($customer_thread_message)}onClick="return confirm(readMessagesConfirmation)"{/if}>
	...
	<form id="dbkPDFForm" target="_blank" class="container-command-top-spacing" action="{$link->getAdminLink('AdminPdf')|escape:'html':'UTF-8'}&amp;submitAction=generatePDFlabels&amp;id_order={$order->id|intval}" method="post">
