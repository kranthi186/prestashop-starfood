...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		// 
		$('.product_discount').live('keyup', function(e){
			var input = $(this);
			//var product = input.attr('rel').split('_');
			var parentTr = input.parents('tr');
			var priceInput = parentTr.find('.product_unit_price');
			var discount = input.val().replace(",",".");
			var price = new Number( priceInput.data('price') );
			var priceNew = null;
			if( discount.search(/\%/) > 0 ){
				discountVal = new Number( discount.replace(/\%/, '') );
				priceNew = price - (price / 100 * discountVal);
			}
			else{
				discountVal = new Number( discount );
				priceNew = price - discountVal;
			}
			...
			var input = $(this);
			input.parents('tr').find('.product_unit_price').change();
		});
		//
		...
		$('#products_table').on('click', 'button.add-product', function(){
			var productId = $(this).data('product_id');
			var attributeId = $(this).data('attribute_id');
			updateQty(productId, attributeId, 0, 1);
		});
	...
	function resetBind()
	{
        ...
        /**
         * Requests central store about expected orders and shows table with orders if they exist
         * 
         * */
        /*function showSupplierOrdersTable(sku)
        {
            // show loader icon
            $('#expectedStock').html('<i class="icon-refresh icon-spin"></i>&nbsp;{l s='Loading' js=1} ...');
            
            // call server
            $('#expectedStock').load('https://www.vipdress.de/admin123/index_service.php/supplier_orders/show_supplier_orders_by_sku/'+
	...
	function displayQtyInStock(id)
	{
		...
		$('#qty_in_stock').html(stock[id_product][id_product_attribute]);
                //showSupplierOrdersTable(supplierReferences[id_product+'_'+id_product_attribute]);
	}
	...
	{
		var customer_query_options = [];
		$('input.customer_query_option:checked').each(function(){
			customer_query_options.push( $(this).val() );
		});
		...
		$.ajax({
			...
			data : {
				ajax: "1",
				//tab: "AdminCustomers",
				action: "searchCustomersByQuery",
				term: $('#customer').val(),
				term_options: customer_query_options
			},
			success : function(res)
			{
				if(res.found)
			...
			success : function(res)
			{
				...
				if(res.found)
				{
					...
					$.each(res.products, function() {
                                                supplierReferences[this.id_product+'_0'] = this.supplier_reference;
						...
						if (this.customizable == '1' || this.customizable == '2')
						{
							...
							customization_html += '</form></div></div>';
                                                ...
                                                // sort combinations
                                                // first move in real array
                                                var sortedCombinations = [];
                                                $.each(this.combinations, function() {
                                                    supplierReferences[id_product+'_'+this.id_product_attribute] = this.supplier_reference;
                                                    sortedCombinations.push(this);
                                                });
                                                sortedCombinations.sort(function(a, b){
                                                    a = a.attributes;
                                                    b = b.attributes;
                                                    return a < b ? -1 : (a > b ? 1 : 0);
                                                });
                                                
						for(var i=0; i<sortedCombinations.length; i++)
                                                {
							attributes_html += '<option rel="'+sortedCombinations[i].qty_in_stock+'" '+(sortedCombinations[i].default_on == 1 ? 'selected="selected"' : '')+' value="'+sortedCombinations[i].id_product_attribute+'">'+sortedCombinations[i].attributes+' - '+sortedCombinations[i].formatted_price+' ('+sortedCombinations[i].qty_in_stock+')</option>';
							stock[id_product][sortedCombinations[i].id_product_attribute] = sortedCombinations[i].qty_in_stock;
						}
	...
	function updateCartProducts(products, gifts, id_address_delivery)
	{
		...
		$.each(products, function() {
			...
			cart_content += '<tr><td><img src="'+this.image_link+'" title="'+this.name+'" /></td>'+
				'<td>'+this.name+'<br />'+this.attributes_small+'</td>'+
				'<td>'+this.supplier_reference+'</td>'+
				'<td>'+this.ean13+'</td>'+
				'<td><input type="text" rel="'+this.id_product+'_'+this.id_product_attribute+'" class="product_unit_price" value="' + this.numeric_price + '" data-price="'+this.numeric_price+'" /></td>';
			cart_content += '<td>'+ (!this.id_customization ? '<div class="input-group fixed-width-md"><div class="input-group-btn"><a href="#" class="btn btn-default increaseqty_product" rel="'+this.id_product+'_'+this.id_product_attribute+'_'+(this.id_customization ? this.id_customization : 0)+'" ><i class="icon-caret-up"></i></a><a href="#" class="btn btn-default decreaseqty_product" rel="'+this.id_product+'_'+this.id_product_attribute+'_'+(this.id_customization ? this.id_customization : 0)+'"><i class="icon-caret-down"></i></a></div>' : '');
			...
			cart_content += (!this.id_customization ? '<div class="input-group-btn"><a href="#" class="delete_product btn btn-default" rel="delete_'+this.id_product+'_'+this.id_product_attribute+'_'+(this.id_customization ? this.id_customization : 0)+'" ><i class="icon-remove text-danger"></i></a></div></div>' : '')+'</td>';
			cart_content +=
				'<td><div class=""><input type="text" rel="'+this.id_product+'_'+this.id_product_attribute+'" class="product_discount form-control" value=""></div></td>'+
				'<td>' + formatCurrency(this.numeric_total, currency_format, currency_sign, currency_blank) + '</td>'+
				'</tr>';
		...
		$('#customer_cart tbody').html(cart_content)/*.promise().done(function(){
			$('.product_discount').typeWatch({
				captureLength: 0,
				highlight: false,
				wait: 0,
				callback: function(e){
	...
	function updateQty(id_product, id_product_attribute, id_customization, qty)
	{
		$.ajax({
			...
			success : function(res)
			{
                var text1 = $('#submitAddProduct').text();
                $('#submitAddProduct').text('{l s='Product has been added'}');
				setTimeout(function() {
					$('#submitAddProduct').text(text1);
				}, 3000);
	...
	var treeClickFunc = function() {
		...
		$.ajax({
			...
			success: function(res){
					...
					if(this.combinations.length){
						for(var ci in this.combinations){
							...
							productRow.append( $('<td/>').html(
								formatCurrency(new Number(this.combinations[ci].price_tax_excl), currency_format, currency_sign, currency_blank)) 
							);
						...
						productRow.append( $('<td/>').html(
							formatCurrency(new Number(this.price_tax_excl), currency_format, currency_sign, currency_blank)
						) );
				...
				products_found += '<label class="control-label col-lg-4">{l s='Product'}</label><div class="col-lg-8"><select id="id_product" onclick="display_product_attributes();display_product_customizations();">';
				attributes_html += '<label class="control-label col-lg-4">{l s='Combination'}</label><div class="col-lg-8">';
				$.each(res.products, function() {
	               	supplierReferences[this.id_product+'_0'] = this.supplier_reference;
					products_found += '<option '+(this.combinations.length > 0 ? 'rel="'+this.qty_in_stock+'"' : '')+' value="'+this.id_product+'">'+this.name+(this.combinations.length == 0 ? ' - '+this.formatted_price : '')+'</option>';
					attributes_html += '<select class="id_product_attribute" id="ipa_'+this.id_product+'" style="display:none;">';
...
<div class="leadin">{block name="leadin"}{/block}</div>
	<div class="panel form-horizontal" id="customer_part">
		...
		<div id="search-customer-form-group" class="form-group">
			...
			<div class="col-lg-12">
				<input checked="checked" class="customer_query_option" type="checkbox" name="scfg[]" value="customer_name" > {l s='Customer Name'}&nbsp;&nbsp;
              	<input checked="checked" class="customer_query_option" type="checkbox" name="scfg[]" value="customer_email" > {l s='Customer Email'}&nbsp;&nbsp;
              	<input checked="checked" class="customer_query_option" type="checkbox" name="scfg[]" value="customer_address" > {l s='Customer address'}&nbsp;&nbsp;
               	<input checked="checked" class="customer_query_option" type="checkbox" name="scfg[]" value="customer_phone" > {l s='Customer Phone'}&nbsp;&nbsp;
               	<input checked="checked" class="customer_query_option" type="checkbox" name="scfg[]" value="product_id" > {l s='Product id'}&nbsp;&nbsp;
               	<input checked="checked" class="customer_query_option" type="checkbox" name="scfg[]" value="supplier_reference" > {l s='Supplier reference'}&nbsp;&nbsp;
               	<input checked="checked" class="customer_query_option" type="checkbox" name="scfg[]" value="product_name" > {l s='Product name'}&nbsp;&nbsp;
               	<input checked="checked" class="customer_query_option" type="checkbox" name="scfg[]" value="invoice_id" > {l s='Invoice id'}&nbsp;&nbsp;
               	<input checked="checked" class="customer_query_option" type="checkbox" name="scfg[]" value="supplier_name" > {l s='Supplier name'}&nbsp;&nbsp;
               	<input checked="checked" class="customer_query_option" type="checkbox" name="scfg[]" value="country_name" > {l s='Country name'}&nbsp;&nbsp;
               	<input checked="checked" class="customer_query_option" type="checkbox" name="scfg[]" value="company_name" > {l s='Company name'}&nbsp;&nbsp;
...
<form class="form-horizontal" action="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}&amp;submitAdd{$table|escape:'html':'UTF-8'}=1" method="post" autocomplete="off">
	<div class="panel" id="products_part" style="display:none;">
		...
		<div class="row">
			<div class="col-lg-7">
				<div id="order_new_cart_products">
				<table class="table" id="customer_cart">
					<thead>
						<tr>
							...
							<th><span class="title_box">{l s='Spl.ref.'}</span></th>
							<th><span class="title_box">{l s='EAN'}</span></th>
			...
			<div class="col-lg-5">
				<div >
						{l s='Categories'}
						<div id="category_block" style="max-height:600px;overflow:auto;">
							{$category_tree}
						</div>
				</div>
			<div class="">
		...
		{*<div class="row">
			...
			<div class="form-group">
				<label class="control-label col-lg-1" for="qty">{l s='Quantity'}</label>
				<div class="col-lg-11" style="width: auto;">
					...
					<p class="help-block">{l s='In stock'} <span id="qty_in_stock"></span></p>
                                ...
                                <div id="expectedStock" style="float: left;margin-left: 10px;margin-top: -20px;"></div>
			</div>
