...
class StockAvailableCore extends ObjectModel
{
    ...
    public static function getStockAvailableIdByProductId($id_product, $id_product_attribute = null, $id_shop = null)
    {
        ...
        $stockId = (int)Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue($query);
        ...
        if (!$stockId)
        {
            ...
            {
                $db->execute('insert into '._DB_PREFIX_.'stock_available (id_product, id_product_attribute, id_shop, quantity) '
                    . 'select '.$id_product.', '.$id_product_attribute.', '.$id_shop.', quantity from '._DB_PREFIX_.
                    'product_attribute where id_product_attribute='.$id_product_attribute);
            }
            else
            {
                $db->execute('insert into '._DB_PREFIX_.'stock_available (id_product, id_product_attribute, id_shop, quantity) '
                    . 'select '.$id_product.', 0, '.$id_shop.', quantity from '._DB_PREFIX_.
                    'product where id_product='.$id_product);
            }
            $stockId = $db->Insert_ID();
        }
        return $stockId;
    }
